...
const today = new Date(), seconds = 1000, minutes = 60, hours= 24, week = 7;
let maxDate = today.toISOString().substring(0, 10);

calculateAge.addEventListener("click", function (e) {
    console.log(ageValue.value, maxDate);
   ...
   let dayTotal = Math.abs(today.getDate() - birthDate.getDate());
    //  console.log(totalYears, monthTotal, dayTotal);
    currentAge.innerText = `${totalYears} Years ${monthTotal} Month ${dayTotal} Days`;
    ...
    let totalMinutea = Math.round(Math.abs(today.getTime()-birthDate.getTime()) / oneMinutes);
    ...
    InMinutes.innerText = totalMinutea ;
    ...
    let totalDaysLeft = Math.round(Math.abs(birthDayThisYear.getTime() - today.getTime()) / oneDay);
